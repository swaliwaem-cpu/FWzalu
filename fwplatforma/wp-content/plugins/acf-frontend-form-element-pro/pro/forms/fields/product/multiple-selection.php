...
class acf_field_multiple_selection extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		$this->label = _x('Multiple Selection', 'noun', FEA_NS);
		$this->public = false;
		$this->defaults = array(
			'multiple' 		=> 0,
			'allow_null' 	=> 0,
		...
		if( ! empty( $field['sub_fields'] ) ){
            ...
            foreach( $field['sub_fields'] as $sub_field ){
                ...
                if( empty($sub_field['placeholder']) ) {
                    $sub_field['placeholder'] = _x('Select', 'verb', FEA_NS);
                }
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Choices',FEA_NS),
			'instructions'	=> __('Enter each choice on a new line.',FEA_NS) . '<br /><br />' . __('For more control, you may specify both a value and label like this:',FEA_NS). '<br /><br />' . __('red : Red',FEA_NS),
			'name'			=> 'choices',
			'type'			=> 'textarea',
		));	
		...
		acf_render_field_setting( $field, array(
			...
			'instructions'	=> __('Enter each default value on a new line',FEA_NS),
			'name'			=> 'default_value',
			'type'			=> 'textarea',
		));
		...
		// return_format
		acf_render_field_setting( $field, array(
			'label'			=> __('Return Format',FEA_NS),
			'instructions'	=> __('Specify the value returned',FEA_NS),
			'type'			=> 'select',
			'name'			=> 'return_format',
			'choices'		=> array(
				'value'			=> __('Value',FEA_NS),
				'label'			=> __('Label',FEA_NS),
				'array'			=> __('Both (Array)',FEA_NS)
			)
		));
