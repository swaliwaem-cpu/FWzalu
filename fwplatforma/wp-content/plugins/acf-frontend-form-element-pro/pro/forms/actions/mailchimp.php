...
class SendMailchimp extends ActionBase{
	...
	public function run( $form, $step = false ){
		...
		if( empty( $record['mailchimp'] ) ) return;

		if( ! is_array( $record['mailchimp'] ) || empty( $record['mailchimp']['email'] ) ) return;
			...
			if( isset( $mc_field[1] ) ){
				...
				if( $field_key == 'status' ){
					if( ! $mc_field['_input'] ){
						...
						if( ! $_field || empty( $_field['save_unsubscribed'] ) ) return;
				...
				if( isset( $result->status ) ){
					if( is_numeric( $result->status ) ){	
						switch ( $result->status ) {
							case 200:
								$response = __( 'Success, newsletter subcribed using mailchimp API', FEA_NS );
								break;
							case 214:
								$response = __( 'Already Subscribed', FEA_NS );
								break;
							default:
								$response = __( 'Oops, please try again.', FEA_NS ) . '<br>[msg_code='.$result->status.']';
								break;
						}
